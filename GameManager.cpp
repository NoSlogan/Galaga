...
{ 
	...
	if (scoreFile.good()) 
	{
		for (int i = 0; i < 10; ++i)
		{
			scoreFile >> highScore[i].first; 
			std::getline(scoreFile, highScore[i].second); 
		}
	}
	else 
	{
		scoreFile.close(); 
		scoreFile.open("scores.txt", std::ios::out | std::ios::trunc); 
		for (int i = 0; i < 10; ++i)
		{ 
			scoreFile << 0; 
			scoreFile << " nobody\n";
			highScore[i].first = 0;
			highScore[i].second = "nobody";
		}
	}
	scoreFile.close();
}
...
void GameManager::Render()
{
	...
	case GS_PLAYEND:
		...
		for (int i = 9; i >= 0; --i)
		{
			if (player->score >= highScore[i].first)
			{
				tmpPlace = i;
			}
		}
		if (tmpPlace < 11) 
		{
			std::cout << "New record! " << tmpPlace + 1 << " place" << std::endl;
			std::cout << "Enter your name: ";
			std::cin >> tmpName;
			for (unsigned i = 9; i > tmpPlace + 1; --i)
			{
				highScore[i] = highScore[i - 1]; 
			}
			highScore[tmpPlace].first = player->score; 
			highScore[tmpPlace].second = " " + tmpName;
			UpdateScoreFile(); 
		}
		gs = GS_SCORE; 
		break;
	case GS_SCORE:
		...
		for (int i = 0; i < 10; ++i)
		{
			std::cout << i + 1 << ".\t" << std::left << highScore[i].first << "\t" << highScore[i].second << std::endl; 
		}
...
void GameManager::UpdateScoreFile()
{
	std::fstream scoreFile;
	scoreFile.open("scores.txt", std::ios::out | std::ios::trunc);
	for (int i = 0; i < 10; ++i)
